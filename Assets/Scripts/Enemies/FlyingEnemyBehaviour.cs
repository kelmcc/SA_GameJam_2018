...
public class FlyingEnemyBehaviour : EnemyBase
{
    ...
    private IEnumerator TakeHit(Vector3 position)
    {
        ...
        if (Life <= 0)
        {
            ...
            int precentage = Random.Range(0, EnemySettings.DyingSound.Length);
            int index = 0;

            //determine 10%
            if (precentage <= EnemySettings.DyingSound.Length/10)
            {
                index = Random.Range(EnemySettings.DyingSound.Length - 2, EnemySettings.DyingSound.Length);
            }
            else
            {                
                index = Random.Range(0, EnemySettings.DyingSound.Length - 2);
            }

            AudioSource.PlayClipAtPoint(EnemySettings.DyingSound[index], Camera.main.transform.position, Random.Range(0.3f, 0.7f));
